...
@section('style')
<link rel="stylesheet" href="{{asset('assets/css/freeQuestion/show.css')}}">
@endsection
@section('content')
<div class="userFreeQuestion main-body"> 
    <div class="QuestionInfo">
       <a href="{{ $freeQuestion['user']['profile_url'] }}"> {{ $freeQuestion['user']['name'] }} <a>
        <span class="freeQuestionScore">امتیاز: {{$freeQuestion['user']['score']}}</span>
    </div>
    <div class="mainQuestionCol">
        <div class="questionCol1">
            @auth   
            @if(auth()->id() !== $freeQuestion['user']['id'])            
                <button class="question-vote-btn"  data-votetype="up">⬆️</button>
            @endif
            <span class="freeQuestion-score" >{{$freeQuestion['score']}}</span> 
            @if(auth()->id() !== $freeQuestion['user']['id'])            
                <button class="question-vote-btn"   data-votetype="down">⬇️ </button> 
            @endif                  
            @endauth
            
        </div>
        <div class="questionCol2">
...
<script>
            ...
            string = string + `<div class="col1">`
                @auth                        
                    ...
                    if(comment.canMarkAsBest)
                    {
                        ...
                        {                                    
                            string = string + `<button class="best-reply-btn" data-isbest="yes" data-commentid="${comment.id}" > `
                            string = string + '✅ بهترین پاسخ' 
                        }
                        else
                        {
                            string = string + `<button class="best-reply-btn" data-isbest="no" data-commentid="${comment.id}" > `
    ...
    $(document).ready(function() {
        ...
        $('.question-vote-btn').click(function() {
            var url = "{{route('freeQuestion.freeQuestion.vote')}}";        
            data = {freeQuestionId : {{ $freeQuestion['id'] }}, 
            ...
            if(result.success)
            {
                ...
                $(this).parent().find('.freeQuestion-score').html(result.vote)
            }
        })
        ...
        function loadComments() {
            ...
            var url = "{{route('freeQuestion.fetchComments')}}";        
        ...
        function rebindVote() {
            ...
            $('.vote-btn').off("click").click(function() {
                var url = "{{route('freeQuestion.freeQuestion.comment.vote')}}";        
        ...
        function rebindBestReplay() {
            ...
            $(".best-reply-btn").off("click").click(function() {
                ...
                $(`.best-reply-btn`).html("انتخاب به عنوان بهترین پاسخ");
                ...
                var url = "{{route('freeQuestion.best-reply')}}";        
